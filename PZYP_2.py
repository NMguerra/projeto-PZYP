...
def compress_file(fich, level, password, summary):
    ...
    if password is None:
        ...
        if os.path.exists(fich):
            ...
            encode(text_File.read(), set_level(int(level)), password) # Runs encode cicle
        ...
        if os.path.exists(fich):
            ...
            if summary == True:
                ...
                compressed_file.close()
            compressed_file = open(fich.rsplit('.', 1)[0]+"."+DEFAULT_EXT, "ab") #opens in append mode to write data
            encode(str(encrypted_data), set_level(int(level)),password) # Runs encode cicle
            compressed_file.close()
            text_File.close()
...
def decompress_file(fich, level, password, summary):
    global compressed_file
    if password is None:
        ...
        if os.path.exists(fich):
            ...
            #print(decode(read_lzs_file(dados)))
            text_File.close()
            file_name = str(fich.rsplit('.', 1)[0]+"_descomprimido.txt")
            #print(file_name)
            key = make_key((password).encode(encoding))
...
def main():
    ...
    if args['--decompress']:
        decompress_file(file, args["--level"], args["--password"],args["--summary"])
